...
def get_deposit_rate():
    r = requests.get(
        "https://money24h.vn/lai-suat-gui-tiet-kiem-ngan-hang", verify=False
    )
    ...
    df.rename(
        columns=lambda col: col.replace("1 Tháng", " 1 Month").replace(
            "Tháng", "Months"
        ),
        inplace=True,
    )
...
@st.experimental_memo
def get_loan_rate():
    r = requests.get("https://money24h.vn/lai-suat-vay-ngan-hang", verify=False)
    soup = BeautifulSoup(r.text, "html.parser")
    div = soup.select("#__NEXT_DATA__")[0].text
...
def format_data(data):
    ...
    df = df.set_index("Bank").T.rename(
        columns=lambda col: col.replace("Ngân hàng ", "")
    )
